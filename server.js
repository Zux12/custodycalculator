// --- Minimal static server (keep if you already have similar) ---
...
app.post("/api/aga8-z", async (req, res) => {
  try {
    ...
    const mod = await import("@sctg/aga8-js");

    let core = null;
    if (typeof mod.default === "function")        core = await mod.default();
    else if (typeof mod.init === "function")      core = await mod.init();
    else if (mod.AGA8)                            core = mod.AGA8;
    else                                          core = mod;
    ...
    if (!(Z > 0) || Z > 2) return res.status(500).json({ ok:false, error:"AGA8 returned invalid Z" });
